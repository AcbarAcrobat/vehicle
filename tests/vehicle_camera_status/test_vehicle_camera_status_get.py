...
class TestVehicleCameraStatusGet:
    ...
    def test_response_contains_only_given_columns(self, endpoint, columns):
        ...
        validate(r.json(), vehicle_camera_status.foreign_key_schema(columns))
    ...
    def test_filter_by_attribute(self, endpoint):
        body = {
            "filter_by": [{"attribute": "bitrate", "operator": ">=", "value": 10}]
        }
        r = endpoint.get(json=body)

        resp = r.json()['result']
        bitrates = [r['bitrate'] for r in resp]

        [AssertThat(i).IsAtLeast(10) for i in bitrates]

    @allure.title("Получение списка экземляров сущности по условию 'И'")
    def test_filter_by_multiple_attributes(self, endpoint):
        body = {
            "filter_by": [
                {"attribute": "bitrate", "operator": ">=", "value": 10},
                {"attribute": "resolution_width", "operator": ">=", "value": 10}
            ]
        }
        r = endpoint.get(json=body)
        ...
        for data in resp:
            AssertThat(data['bitrate']).IsAtLeast(10)
            AssertThat(data['resolution_width']).IsAtLeast(10)
