...
from schema import vehicle_type
from schema.base import skeleton, build
from truth.truth import AssertThat


class TestVehicleTypeGet:

    @allure.title("Схема ответа")
    def test_schema(self, endpoint):
        r = endpoint.get()
        validate(r.json(), vehicle_type.foreign_key_schema())
    ...
    def test_response_contains_only_given_columns(self, endpoint, columns):
        ...
        validate(r.json(), vehicle_type.foreign_key_schema(columns))
    ...
    def test_filter_by_attribute(self, endpoint, data_vehicle_type):
        id_ = data_vehicle_type["ids"][0]
        body = {
    ...
    def test_filter_by_multiple_attributes(self, endpoint, data_vehicle_type):
        id_ = data_vehicle_type["ids"][0]
        name = data_vehicle_type["body"]["values"][0]["name"]
        body = {
            "filter_by": [
    ...
    def test_search_by(self, endpoint, data_vehicle_type):
        id_ = data_vehicle_type["ids"][0]
        name = data_vehicle_type["body"]["values"][0]["name"]
        body = {
            "search_by": [
