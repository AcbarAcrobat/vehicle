...
from schema import vehicle_status
from schema.base import skeleton, build
from truth.truth import AssertThat


class TestVehicleStatusGet:

    @allure.title("Схема ответа")
    def test_schema(self, endpoint):
        r = endpoint.get()
        validate(r.json(), vehicle_status.foreign_key_schema())
    ...
    def test_response_contains_only_given_columns(self, endpoint, columns):
        ...
        validate(r.json(), vehicle_status.foreign_key_schema(columns))
    ...
    def test_filter_by_attribute(self, endpoint):
        body = {
            "filter_by": [{"attribute": "cameras_total", "operator": "<=", "value": 10}]
        }
        r = endpoint.get(json=body)

        resp = r.json()['result']
        cameras = [r['cameras_total'] for r in resp]

        [AssertThat(i).IsAtMost(10) for i in cameras]

    @allure.title("Получение списка экземляров сущности по условию 'И'")
    def test_filter_by_multiple_attributes(self, endpoint):
        body = {
            "filter_by": [
                {"attribute": "cameras_total", "operator": "<=", "value": 10},
                {"attribute": "ping", "operator": "<=", "value": 50}
            ]
        }
        r = endpoint.get(json=body)
        ...
        for data in resp:
            AssertThat(data['cameras_total']).IsAtMost(10)
            AssertThat(data['ping']).IsAtMost(50)
    ...
    def test_search_by(self, endpoint):
        ...
        for data in resp:
            ...
            ping = data['ping']
            if cameras_total > 10 and ping > 50:
                raise AssertionError(f'Not true that {cameras_total} <= 10 OR {ping} <= 50')
