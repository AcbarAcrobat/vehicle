...
from schema import vehicle_to_attached_file
from schema.base import skeleton, build
from truth.truth import AssertThat


class TestVehicleToAttachedFileGet:

    @allure.title("Схема ответа")
    def test_schema(self, endpoint):
        r = endpoint.get()
        validate(r.json(), vehicle_to_attached_file.foreign_key_schema())
    ...
    def test_response_contains_only_given_columns(self, endpoint, columns):
        ...
        validate(r.json(), vehicle_to_attached_file.foreign_key_schema(columns))
    ...
    def test_filter_by_multiple_attributes(self, endpoint):
        body = {
            "filter_by": [
                ...
                {"attribute": "vehicle_id", "operator": "<=", "value": 200}
            ]
        }
        r = endpoint.get(json=body)
        ...
        for data in resp:
            ...
            AssertThat(data['vehicle_id']).IsAtMost(200)
    ...
    def test_search_by(self, endpoint):
        ...
        for data in resp:
            ...
            if id_ > 10 and vehicle_id > 200:
                raise AssertionError(f'Not true that {id_} <= 10 OR {vehicle_id} <= 200')
